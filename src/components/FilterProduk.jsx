  ...
  return (
    <div className="px-6 md:px-12 lg:px-16 xl:px-28">
      <div className="py-16 flex justify-center items-center gap-3">
        <button onClick={toggleFilters} className="flex items-center justify-center w-[10%] max-w-11 aspect-square rounded-md text-xl text-nadia-400 border border-gray-300 active:scale-95 transition-all ease-in-out">
      ...
      {/* Filter Slider */}
      <div className={`transition-transform duration-300 ease-in-out transform ${showFilters ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-lg p-6`}>
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-lg font-semibold">Filter Products</h2>
          <button onClick={() => setShowFilters(false)} className="text-black text-2xl font-bold">
            &times;
          </button>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {["decorative mirror", "jujukhat", "decorative table", "decorative lights", "wall decoration", "bag"].map((item) => (
              ...
              >
                <input type="radio" name="category" value={item} checked={category === item} onChange={handleChage} className="hidden peer" />
                {item}
              </label>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {["rope agel", "rapia agel", "wood", "quill"].map((item) => (
              <label
                ...
                className="relative flex items-center justify-center p-2 bg-[#FAF7F0] rounded cursor-pointer text-sm font-medium border-2 border-transparent peer-checked:bg-blue-500 peer-checked:text-white peer-checked:border-blue-500"
              >
                <input type="radio" name="material" value={item} checked={material === item} onChange={handleChage} className="hidden peer" />
                {item}
              </label>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {["30", "40", "60", "70", "1 meter"].map((item) => (
              <label
                ...
                className="relative flex items-center justify-center p-2 bg-[#FAF7F0] rounded cursor-pointer text-sm font-medium border-2 border-transparent peer-checked:bg-blue-500 peer-checked:text-white peer-checked:border-blue-500"
              >
                <input type="radio" name="size" value={item} checked={size === item} onChange={handleChage} className="hidden peer" />
                {item}
              </label>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <label key={value} className={`inline-flex items-center space-x-2 p-3  bg-[#FAF7F0] cursor-pointer rounded-lg text-sm font-medium`} onClick={() => handleChage({ target: { value } })}>
                <input type="radio" name="color" value={value} checked={color === value} onChange={handleChage} className="form-radio" style={{ display: "none" }} />
                <span className="inline-block w-5 h-5 rounded-full" style={{ backgroundColor: colorCode }}></span>
                <span>{name}</span>
              </label>
            ))}
          </div>
        ...
        {/* Reset Button */}
        <button onClick={resetFilters} className="mt-7 reset-button px-6 py-3 bg-nadia-more400 text-white rounded-lg w-full">
          Reset Filters
        </button>
      </div>
