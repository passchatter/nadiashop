...
function Detail() {
    ...
    useEffect(() => {
        ...
        const getDetail = async () => {
          try {
            const response = await axios.get(
              `http://localhost:8888/nadiashop/backend/api/produk/detail/${id}`
            );
        ...
        const getRelatedProducts = async (category, productId) => {
          try {
            const response = await axios.get(
              `http://localhost:8888/nadiashop/backend/api/produk/related/${category}/${productId}`
            );
            ...
            <div className="max-w-6xl mx-auto px-6 py-10">
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <span className="text-gray-600">{produk.size}</span>
                            </div>
                            <div>
                                <span className="font-semibold text-gray-700">Material:</span>{" "}
                                <span className="text-gray-600">{produk.material}</span>
                            </div>
                            <div>
                                <span className="font-semibold text-gray-700">Color:</span>{" "}
                                <span className="text-gray-600">{produk.color}</span>
                            </div>
                            <div>
                                <span className="font-semibold text-gray-700">Category:</span>{" "}
                                <span className="text-gray-600">{produk.category}</span>
                            </div>
                        </div>

                        {/* Tombol */}
                        <button className="mt-4 px-6 py-3 bg-nadia-more400 text-white font-semibold rounded-lg transition">
                           Order Now
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    {relatedProduk.map((relatedProduct) => (
                        ...
                        >
                        <img
                            src={`http://localhost:8888/nadiashop/backend/image/${relatedProduct.image}`}
                        ...
                        <div className="p-4">
                            ...
                            <a
                            href={`/product/detail/${relatedProduct.id}`}
